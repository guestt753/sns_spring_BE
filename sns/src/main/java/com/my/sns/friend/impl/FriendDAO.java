...
public class FriendDAO {
	// JDBC 관련 변수
		...
		private final String SELECT_FRIEND_BY_USERNO = "select user_one_no, friend_status from friend where user_one_no = ? or user_two_no = ?;";
		private final String DELETE_FRIEND_REQUEST_BY_USERNO = "delete from friend_request where user_two_no = ? and user_one_no = ?";
		private final String SELECT_FRIEND_REQUEST_BY_USERNO = "select u.user_no, u.user_name, u.user_image_url from user u join friend_request rq on rq.user_one_no = u.user_no where rq.user_two_no = ?;";
		private final String INSERT_FRIEND = "insert into friend(user_one_no, user_two_no) values(?,?)";
